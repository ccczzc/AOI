...
void *Receiver(void *sock) {
  ...
  while (1) {
    ...
    fileYLog << getDoubleTimeNow() << "\t" 
             << RTT << "\t"
             << systemTime[receiver_seq] << "\t" 
             << depTime << "\t"
             << controlPacketDelay[controlIndex - 1] << "\t"
             << controlDepartureTime[controlIndex - 1] << "\t"
             << controlIndex << "\t" 
             << currentReceiveTime << "\t" 
             << currentControlTime << "\t"
             << std::endl;

    fileRxLog << getDoubleTimeNow() << "\t" << receiver_seq << "\t"
              << systemTime[receiver_seq] << endl;
  }
}
...
int main(int argc, char *argv[]) {
  ...
  this_thread::sleep_for(
      chrono::seconds(3)); // hang on for last packets to come back :P
