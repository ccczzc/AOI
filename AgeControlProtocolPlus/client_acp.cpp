...
//`./client localhost 526 500 2 2 49050` to run it
#include <arpa/inet.h>
#include <cstdlib>
#include <ctime>
#include <fstream>
...
static uint32_t transmission_seq = 0;
static uint32_t receiver_seq = 0;
int lastSeqNum = 0;
...
static void getBacklogArrival(time_t sampleTime) {
  ...
  filebacklogArr << getDoubleTimeNow() << "\t"
                 << globalBacklog[backlogIndex - 1] << "\t"
                 << backlogTime[backlogIndex - 1] << "\t" << backlogIndex
                 << std::endl;
}

static void getAverageBacklog(time_t backlogTime_local[],
                              double globalBacklog_local[],
                              int backlogIndex_local,
                              time_t currentBacklogTime) {
  double sumBacklog = 0;
  time_t sumTime = 0;
    ...
    for (int i = 0; i < backlogIndex_local; i++) {
      sumBacklog = sumBacklog +
                   globalBacklog_local[i] *
                       (backlogTime_local[i + 1] - backlogTime_local[i]) * 1e-9;
    }
  ...
  fileAvgbacklog << getDoubleTimeNow() << "\t" << sumBacklog << "\t"
                 << currentAverageBacklog << "\t" << sumTime << "\t"
                 << currentBacklogTime << "\t" << backlogIndex_local << "\t"
                 << std::endl;
  if (backlogIndex_local == 1) {
...
static void getAgeEstimate(time_t controlPacketDelay_local[],
                           time_t controlDepartureTime_local[],
                           int controlIndex_local,
                           time_t currentControlTime_local,
                           time_t prevReceiveTime_local,
                           time_t lastDelay_local) {
  ...
  if (controlIndex_local > 1) {
    ...
    for (int i = 0; i < controlIndex_local; i++) {
      ageEstimate =
          ageEstimate +
          (controlPacketDelay_local[i] * controlDepartureTime_local[i + 1]) +
          (0.5 * controlDepartureTime_local[i + 1] *
           controlDepartureTime_local[i + 1]);

      fileDebugAge << getDoubleTimeNow() << "\t" << ageEstimate << "\t"
                   << controlPacketDelay_local[i] << "\t"
                   << controlDepartureTime_local[i + 1] << "\t" << std::endl;
    }
  ...
  fileAgeEst << getDoubleTimeNow() << "\t" << age_estimate[update_index - 1]
             << "\t" << ageEstimate << "\t" << sum_denominator << "\t"
             << controlPacketDelay_local[controlIndex_local - 1] << "\t"
             << controlDepartureTime_local[controlIndex_local - 1] << "\t"
             << std::endl;
...
static void *controlAction(void *x1) {
  ...
  backlogTime_local[backlogIndex] =
      currentBacklogTime; // make a copy to both local and global server
  ...
  getAgeEstimate(controlPacketDelay_local, controlDepartureTime_local,
                 controlIndex_local, currentControlTime, prevReceiveTime_local,
                 lastDelay);
  // printf("GetBacklog: ControlAction Block\n");
  getAverageBacklog(backlogTime_local, globalBacklog_local, backlogIndex_local,
                    currentBacklogTime);
  ...
  if ((recent_age_estimate - prev_age_estimate >= 0) && (changeinBacklog > 0)) {
    if (current_action == 1) {
      ...
      desiredChangeinLambda =
          ((n - 1) * currentAverageBacklog / (n * RTT_local));

      calcLambda = 1e9 * ((1.0 / depTime_local) - desiredChangeinLambda);
    ...
    current_action = 1;
  } else if ((recent_age_estimate - prev_age_estimate >= 0) &&
             (changeinBacklog <= 0)) {
    // case b
    calcLambda = (1e9 / depTime_local) + (1e9 * stepSize / RTT_local);
    current_action = 2;
...
void *Receiver(void *sock) {
  ...
  while (1) {
    ...
    systemTime[receiver_seq] =
        getDoubleTimeDiff(&tx[receiver_seq], &rx[receiver_seq]);
    ...
    if (controlTaken == true) {
      controlDepartureTime[controlIndex] =
          currentReceiveTime - currentControlTime;
      controlTaken = false;
    } else {
    ...
    fileYLog << getDoubleTimeNow() << "\t" << RTT << "\t" << depTime << "\t"
             << controlPacketDelay[controlIndex - 1] << "\t"
             << controlDepartureTime[controlIndex - 1] << "\t" << controlIndex
             << "\t" << currentReceiveTime << "\t" << currentControlTime << "\t"
             << std::endl;
  }
}
...
time_t ping_to_get_rtt(int sockfd, struct addrinfo *p) {
  ...
  for (seqNum = 0; seqNum < 10; seqNum++) {
    ...
    memset(buf, 0, sizeof(buf));
    addr_len = sizeof their_addr;
    if (recvfrom(sockfd, buf, MAXBUFLEN - 1, 0, (struct sockaddr *)&their_addr,
                 &addr_len) == -1) {
      perror("recvfrom");
      exit(1);
    }
...
int main(int argc, char *argv[]) {
  ...
  for (int seqNum = 0;; seqNum++) { // message_num times
    ...
    mtx.unlock();
    // fileintArrLog << getDoubleTimeNow() << '\t' << arrivalTime << '\t' <<
    // seqNum << '\n'; filelambdaLog << getDoubleTimeNow() << '\t' << interRate
    // << '\n';

    // this_thread::sleep_for(chrono::nanoseconds((long long
  ...
  // this_thread::sleep_for(chrono::seconds(3));//hang on for last packets to
  // come back :P
